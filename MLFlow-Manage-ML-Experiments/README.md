...
When we conduct a run with mlflow, a directory called mlruns is created and the tracking will be stored in this directory. Change the URI pointed to using this mlflow.set_tracking_uri().
...
`mlflow models serve -m /Users/jinyapeh/Desktop/Complete-MLOps-BootCamp/MLFlow-Manage-ML-Experiments/mlruns/386488476667344857/dda2ffe73c524b54bc0dd5da9f7023ac/artifacts/RandomForestClassifier --port 9000 --no-conda` 

if error "mlflow.exceptions.MlflowException: Could not find the pyenv binary. See https://github.com/pyenv-win/pyenv-win#installation for installation instructions." persists.
...
(NOTE:) Instructions to run this command: 
1. First create a schema called `db_mlflow` on MySQL
2. Enter command: mlflow server --host <host_ip> --port <port_number> --backend-store-uri mysql://<mysql_user>:<mysql_password>@<connection_address_to_db>/<schema_name> --default-artifact-root <directory_where_artifacts_are_stored>
...
(NOTE:) Instructions:
Need to run this command in the CLI too when we change the tracking URI in the code as well
Ensure that it points to port 5001 where resources are located at

`mlflow models serve -m "models:/Prediction_Model_RF/Production" -p 1234`

(NOTE:) Syntax: mlflow models serve -m "models:/<model_name>/<model_env_name>" -p <port_number> where model_env_name = {'Staging', 'Production'}