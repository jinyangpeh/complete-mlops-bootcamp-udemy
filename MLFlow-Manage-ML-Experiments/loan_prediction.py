'''
Note about MLProject

- Entry point scripts listed in the MLProject file can be triggered via the `mlflow run /path/to/MLProject`
- In the case where mlflow is in same directory where this function is called, simply call `mlflow run .`
- The environment and its dependencies will be created by the mlflow backend before running the scripts listed in the entry point

- To run from Git, run `mlflow run <git link> --experiment-name <experiment name>
...
# n_jobs = 1: consider only the most optimal model by taking
...
def mlflow_logging(model, X, y, name):
    ...
    # create a run
     with mlflow.start_run() as run:
        # Comment out to use default localhost port 500
        mlflow.set_tracking_uri("http://0.0.0.0:5001/")
